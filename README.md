...
**3. Zero external dependencies.** Every skill is built natively, inspired by best-in-class open source projects. No API keys, no SaaS subscriptions, no vendor lock-in.
...
That's it. All 12 agents, 33 commands, and 52 skills are now available. Works in any project — new or existing.
...
This auto-creates a `CLAUDE.md` in your project that activates Wantan as orchestrator with agent delegation. Without this file, Claude works normally but won't delegate to the specialized agents.
...
### Activating Agents in Existing Projects

If agents aren't delegating (Claude writes code directly instead of dispatching Tanjiro), create a `CLAUDE.md` in your project root:
...
| Task | Delegate To |
|------|------------|
| Build features, scaffold, API, database, auth | **Tanjiro** |
| UI design, aesthetics, components | **Ochaco** |
| Code review | **Levi** |
| Testing | **Killua** |
| Security scanning | **Itachi** |
| CI/CD, deploys | **Shikamaru** |
| Documentation | **L** |
| Sprint tracking | **Erwin** |
| Research | **Hange** |
| Architecture | **Senku** |
```

This is required because Claude Code plugins provide tools and skills but don't override Claude's default behavior. The `CLAUDE.md` tells Claude to adopt the Wantan persona and delegate.
...
- Routine ops → filtered out (noise)
