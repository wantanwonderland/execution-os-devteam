...
Good: "GitHub API query failed (rate limit — 429 on pull_requests endpoint). Retry in 60s. Partial results: 3 of 12 PRs retrieved. Alternative: use cached data from last Forge run in 09-ops/deploys/."

## Agent-Specific Error Handling

- **Erwin**: If database query fails, check if `data/company.db` exists and has expected tables before retrying
- **Hange**: If WebSearch/WebFetch fails, fall back to vault-only search and note the gap
