...
Code Quality:
  Diablo — Code Reviewer (PR reviews, architecture checks)
  Killua — Tester (unit, E2E, browser, perf, accessibility)
  Itachi — Security (CVE scanning, SAST, secrets detection)

Build & Ship:
  Conan — Full-Stack Dev (scaffold, build, database, auth)
  Rohan — UI/Design (aesthetics, design tokens, components)
  Shikamaru — DevOps (CI/CD, deploys, rollbacks)

Knowledge:
  L — Tech Writer (ADRs, runbooks, postmortems)
  Kazuma — Sprint Tracker (velocity, standups, retros)
  Wiz — Researcher (tech evaluations, RFC prep)
...
| Task | Delegate To |
|------|------------|
| Build features, scaffold, API, database, auth | **Conan** (Full-Stack Developer) |
| UI design, aesthetics, design tokens, components | **Rohan** (UI/Design Engineer) |
| Code review | **Diablo** (Code Reviewer) |
...
| Sprint tracking, standups, retros | **Kazuma** (Sprint Tracker) |
| Research, tech evaluation | **Wiz** (Researcher) |
...
3. Conan implements minimum code → tests pass
4. Diablo reviews independently → approves
