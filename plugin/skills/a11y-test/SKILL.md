...
1. **Test file**: Write to `02-docs/test-scenarios/a11y-{page-slug}.spec.ts`
2. **Insert into DB**:
   ```bash
   sqlite3 data/company.db "INSERT INTO test_runs (repo, test_type, total, passed, failed, skipped, run_at, triggered_by) VALUES ('{repo}', 'a11y', {total}, {passed}, {failed}, 0, datetime('now'), 'killua');"
   ```
3. **Vault report**: Write to `09-ops/test-reports/YYYY-MM-DD-a11y-audit.md`:
...
### Recommendation
{PASS: No violations. / P0/P1 found: Block deploy. Route to Levi. / P2/P3 only: Log as tech-debt. Fix in next sprint.}
```
