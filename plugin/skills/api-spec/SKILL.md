...
1. Write spec to `vault/02-docs/api-specs/{service-name}-openapi.yaml`
2. Add frontmatter wrapper to vault index file at `vault/02-docs/api-specs/README.md` (create if absent)
...
4. **Cwizd endpoints**: present in both → deep compare:
...
Write to `vault/02-docs/api-docs/{service-name}-api-reference.md` with full frontmatter.
...
3. Log breaking changes in `vault/02-docs/api-specs/CHANGELOG.md`
...
- Specs are written to `vault/02-docs/api-specs/` — L does not write to other directories
