...
1. **Test file**: Write to `02-docs/test-scenarios/visual-regression-{page-slug}.spec.ts`
...
4. **Insert into DB**:
   ```bash
   sqlite3 data/company.db "INSERT INTO test_runs (repo, test_type, total, passed, failed, skipped, run_at, triggered_by) VALUES ('{repo}', 'visual', {total}, {passed}, {failed}, 0, datetime('now'), 'killua');"
   ```
5. **Vault report**: Write to `09-ops/test-reports/YYYY-MM-DD-visual-regression.md`:
...
### Recommendation
{PASS: No visual regressions. / FAIL: Review diffs. Route to Levi before merge.}
```
