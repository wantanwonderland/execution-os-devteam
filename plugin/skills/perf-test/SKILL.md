...
Save baseline summary to `09-ops/perf-baselines/YYYY-MM-DD-{endpoint-slug}-baseline.json`:
...
BASELINE_P95=$(cat 09-ops/perf-baselines/{endpoint-slug}-baseline.json | jq '.metrics.p95_ms')
...
1. **k6 script**: Save to `02-docs/perf-tests/{endpoint-slug}.js`
2. **Baseline** (if captured): Save to `09-ops/perf-baselines/YYYY-MM-DD-{endpoint-slug}-baseline.json`
3. **Insert into DB**:
   ```bash
   sqlite3 data/company.db "INSERT INTO test_runs (repo, test_type, total, passed, failed, skipped, run_at, triggered_by) VALUES ('{repo}', 'perf', {total}, {passed}, {failed}, 0, datetime('now'), 'killua');"
   ```
4. **Vault report**: Write to `09-ops/test-reports/YYYY-MM-DD-perf-test.md`:
...
### Recommendation
{GREEN: No regressions detected. Safe to deploy. / YELLOW: Investigate before deploy. / RED: Block deploy. Route to Levi.}
```
...
- Baseline files are stored in `09-ops/perf-baselines/` — never delete them
