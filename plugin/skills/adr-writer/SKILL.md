...
1. Search vault for related decisions: `vault/04-decisions/log/` files with similar tags
2. Search ADRs: `vault/02-docs/adr/` for precedents
3. If the decision involves a library/framework, research alternatives

## Step 2: Determine ADR Number

```bash
ls vault/02-docs/adr/ | grep -oP 'adr-\K[0-9]+' | sort -n | tail -1
```
...
1. Save to `vault/02-docs/adr/YYYY-MM-DD-adr-{NNN}-{title-slug}.md`
