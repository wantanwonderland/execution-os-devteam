...
Killua receives one or more test scenarios from `02-docs/test-scenarios/`. Each scenario has:
...
### Recommendation
{Based on On Failure instructions in scenario: route to Levi, block deploy, flag in standup}
```

### 4. Write Results
1. **Database**: Insert into `test_runs` table via sqlite3:
   ```bash
   sqlite3 data/company.db "INSERT INTO test_runs (repo, test_type, total, passed, failed, skipped, run_at, triggered_by) VALUES ('{repo}', 'browser', {total}, {passed}, {failed}, 0, datetime('now'), 'killua');"
   ```
2. **Vault report**: Write to `09-ops/test-reports/YYYY-MM-DD-test-summary.md` with full frontmatter
...
- Wantan decides whether to dispatch Levi, Shikamaru, or flag in standup
