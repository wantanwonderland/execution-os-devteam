...
`vault/06-ceremonies/pulse-YYYY-WNN.md` — e.g., `pulse-2026-W11.md`
...
|--------|---------------|
| Sprint goal completion | Query `vault/data/company.db` sprints table — `goal_completion_pct` for current sprint |
...
| Standup compliance | Count `vault/06-ceremonies/standup/` files for current week's weekdays (Mon-Fri). Report as N/5. |
| Decisions logged | Count files in `vault/04-decisions/log/` where `created` frontmatter date falls within current week |
