---
name: tech-radar
description: Hange's ThoughtWorks-format tech radar skill. Track technology adoption decisions in four rings, display the current radar, and add or move entries with rationale.
---

# Tech Radar Workflow

When Hange is dispatched for tech radar management, follow this workflow.

## Overview

The tech radar is a living map of technology decisions. It tracks where each technology stands: actively adopted, under trial, being assessed, or on hold. It is grounded in actual decisions made by the team — every entry must link to a decision record in `04-decisions/log/`.
...
Read all decision records in `04-decisions/log/` where `type: tech-radar`:

```bash
grep -rl "type: tech-radar" 04-decisions/log/
```
...
Search `04-decisions/log/` for an existing entry with the same technology name:

```bash
grep -rl "{tech-name}" 04-decisions/log/
```
...
Write a new file to `04-decisions/log/YYYY-MM-DD-radar-{tech-slug}.md`:
...
```bash
grep -rl "{tech-name}" 04-decisions/log/ 03-research/
```

### Step 2: Research Brief (via Hange's primary role)
...
- Every radar entry MUST link to a decision record in `04-decisions/log/`
...
- Assessments without research are guesses — Hange always researches before recommending
