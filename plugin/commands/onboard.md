Generate a codebase orientation guide for a repository. Wiz analyzes and writes the guide.
...
Find the repo:
1. Check `vault/01-projects/` for a matching project file
2. If a local path is given: verify it exists
3. If neither: ask Wantan for the repo location before proceeding

### Step 2: Dispatch Wiz — Codebase Analysis

Wiz performs a full orientation analysis using the codebase-guide skill (`.claude/skills/codebase-guide/SKILL.md`).
...
While Wiz runs, load team-produced context from the vault:

1. **Recent ADRs**: Read all files in `vault/02-docs/adr/` — focus on those tagged to this repo or with recent `created:` dates
2. **Active tech debt**: Query `vault/data/company.db`:
   ```sql
   ...
   ```
3. **Test coverage**: Read `vault/02-docs/test-scenarios/` for scenarios tagged to this repo. Note which flows have coverage and which don't.
4. **Recent incidents**: Query `vault/data/company.db`:
   ```sql
...
Combine Wiz's analysis and vault context into a structured guide.

Save to: `vault/02-docs/onboarding/YYYY-MM-DD-{repo-slug}-guide.md`
...
- **Code questions**: Tag Diablo in your PR for review
- **Architecture questions**: Check `vault/02-docs/adr/` first; surface to Wantan if not answered
- **Incidents**: Follow `vault/02-docs/runbooks/` for operational procedures
```
...
Report:
```
Onboarding guide written: vault/02-docs/onboarding/YYYY-MM-DD-{repo-slug}-guide.md
...
- If `vault/02-docs/onboarding/` does not exist, create it before writing the guide
