...
4. **Last sprint review**: Find the most recent review file in `vault/06-ceremonies/sprint-review/`. Show date and sprint number if present.

5. **Recent activity**: List the 5 most recently modified `.md` files across the vault with their paths and modification dates.

6. **Inbox status**: Count files in `vault/08-inbox/captures/` and `vault/08-inbox/ideas/`. Flag if inbox has more than 10 unsorted items.
