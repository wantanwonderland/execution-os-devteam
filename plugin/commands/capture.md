Quick capture to the vault inbox. Create a new file in `08-inbox/captures/` from the user's input.
...
2. Create the file in `08-inbox/captures/` with this frontmatter:
...
5. **Sort suggestion**: Based on auto-sort rules in CLAUDE.md, suggest where this capture should eventually be moved. Example: "This looks like a decision — consider moving to `04-decisions/log/` or run `/decide` to log it properly."
