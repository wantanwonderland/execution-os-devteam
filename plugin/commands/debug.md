Debug an error or unexpected behavior. Hange + Levi collaboration.
...
| Error Type | Route To |
|-----------|----------|
| External library error, unknown API behavior, third-party SDK issue | Hange |
| Code in active PRs or modules recently reviewed | Levi |
| Both internal code AND external dependency | Hange first, then Levi |

**Dispatch Hange** (if external library involved):
...
**Dispatch Levi** (if error is in codebase code):
...
| Hange research | {external finding if applicable} |
| Levi analysis | {code finding if applicable} |
...
- If the stack trace points to node_modules or vendor code, Hange takes primary
