...
1. **Latest test run**: Query `test_runs` table in `vault/data/company.db` for the most recent entry. Show: date, trigger, test type, pass/fail/skip counts, coverage percentage.

2. **Available scenarios**: List all `.md` files in `vault/02-docs/test-scenarios/` (excluding _template.md). For each, show: title, priority, project, status. Group by priority (critical first).

3. **Coverage gaps**: Check which projects in `vault/01-projects/` have test scenarios vs which don't. Flag projects with zero scenarios.
...
1. **Select scenarios**: Read all `.md` files in `vault/02-docs/test-scenarios/` where `status: active`. If a tag was provided, filter to scenarios containing that tag.
...
4. **Write results**:
   ...
   - Write vault report to `vault/09-ops/test-reports/YYYY-MM-DD-test-summary.md`

5. **Route failures**: If any tests failed, surface them:
   - Severity critical → "Block deploy. Route to Shikamaru."
   - Severity high → "Route to Diablo for PR review."
   - Severity medium → "Flag in next standup."

### Mode: Results

1. Query `test_runs` table for last 5 runs. Show: date, type, pass/fail counts, triggered_by.
2. If the most recent run has failures, show the failure details from `vault/09-ops/test-reports/`.

### Mode: Coverage

1. List all projects from `vault/01-projects/`.
2. For each project, count scenarios in `vault/02-docs/test-scenarios/` matching that project.
...
5. **Write results**: Insert into `test_runs` (test_type='unit'). Write vault report to `vault/09-ops/test-reports/YYYY-MM-DD-unit-tests.md`.
...
5. **Write results**: Insert into `test_runs` (test_type='integration'). Write vault report to `vault/09-ops/test-reports/YYYY-MM-DD-integration-{service}.md`.
...
2. **Check for existing baseline**: Look in `vault/09-ops/perf-baselines/` for a saved baseline for this endpoint.
...
4. **Run k6**:
   ```bash
   k6 run --env BASE_URL={url} --out json=/tmp/perf-result.json vault/02-docs/perf-tests/{endpoint-slug}.js
   ```
...
6. **Write results**: Insert into `test_runs` (test_type='perf'). Write vault report to `vault/09-ops/test-reports/YYYY-MM-DD-perf-{endpoint-slug}.md`.

7. **Route failures**: If thresholds fail → "Performance regression detected. Route to Diablo before deploy."
...
5. **Write results**: Insert into `test_runs` (test_type='visual'). Write vault report to `vault/09-ops/test-reports/YYYY-MM-DD-visual-{page-slug}.md`.

6. **Route failures**: Visual diff found → "Visual regression detected on {viewport}. Review diff at {path}. Route to Diablo."
...
5. **Write results**: Insert into `test_runs` (test_type='a11y'). Write vault report to `vault/09-ops/test-reports/YYYY-MM-DD-a11y-{page-slug}.md`.

6. **Route failures**:
   - Critical/Serious → "Block deploy. Accessibility violations prevent users from completing tasks. Route to Diablo."
   - Moderate/Minor → "Log as tech-debt. Fix in next sprint. File in `vault/04-decisions/log/`."
