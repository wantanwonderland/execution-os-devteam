Analyze and refactor a file or module. Diablo + Senku collaboration.
...
### Step 2: Dispatch Diablo — Code Smell Detection

Diablo reads the target and identifies code smells. Look for:
...
Diablo returns a ranked list of smells: CRITICAL (blocks readability/maintainability) → WARNING (worth fixing) → NIT (optional).

### Step 3: Dispatch Senku — Architecture Improvement Proposals

Senku reads the target and Diablo's smell report, then proposes architectural improvements:
...
Compile Diablo and Senku's findings into a refactoring plan:
...
**Why**: {Diablo smell that motivates this}
