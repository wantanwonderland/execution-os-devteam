...
1. **Identify repos**: List directories in `vault/01-projects/` to find active repos. If the working directory is a repo root, use that.

2. **Dispatch Diablo** with:
...
Same as default but scoped to the specified directory. Diablo analyzes only that directory and its subdirectories. Present the bus factor and knowledge gap reports scoped to that path.
...
4. **Dispatch Diablo** to write `CODEOWNERS` to the repo root once confirmed.

5. **Write vault reports**:
   - Bus factor report → `vault/09-ops/ownership/YYYY-MM-DD-bus-factor.md`
   - Knowledge gap report → `vault/09-ops/ownership/YYYY-MM-DD-knowledge-gaps.md`
