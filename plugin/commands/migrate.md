...
```bash
ls 02-docs/migrations/ | grep -oP '^\d+' | sort -n | tail -1
```
...
Create `02-docs/migrations/{NNNN}-{description-slug}.{ext}` with this structure:
...
```bash
sqlite3 :memory: < 02-docs/migrations/{NNNN}-{description-slug}.sql
```
...
Report:
```
Migration created: 02-docs/migrations/{NNNN}-{description-slug}.{ext}
Type: {sql/ts/py}
...
1. List migration files in `02-docs/migrations/` sorted by number
2. Query `data/company.db` migrations table (if exists) for already-applied migrations:
   ```sql
...
4. For each pending migration (in order):
   ...
   - On approval: execute against `data/company.db`
...
4. On approval: execute the DOWN section against `data/company.db`
...
- If `02-docs/migrations/` does not exist, create it with a `.gitkeep` before proceeding
