...
1. **Open PRs**: Dispatch Levi to run `gh pr list --state open --json number,title,author,createdAt,reviewRequests,labels` for all repos in `01-projects/`.
...
3. **Blockers**: Identify PRs where:
   - Changes requested but no update from author (>24h)
