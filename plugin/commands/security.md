...
1. **Security posture**: Query `security_scans` table in `vault/data/company.db` for the most recent scan per repo. Show:
...
2. **Open alerts**: Dispatch Itachi to run `gh api repos/{owner}/{repo}/dependabot/alerts --jq '[.[] | select(.state=="open")] | length'` for each repo in `vault/01-projects/`. Show alert counts.
...
3. **Write results**:
   ...
   - Write vault report to `vault/09-ops/security-reports/YYYY-MM-DD-security-{repo}.md`
