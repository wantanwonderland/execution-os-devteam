...
1. **Today's commitments check**: Read today's standup log from `vault/06-ceremonies/standup/`. For each commitment, ask: "Done, in-progress, or blocked?"
...
4. **Decision capture**: Ask: "Did you make any decisions today that should be logged?" If yes, capture to `vault/04-decisions/log/`.

5. **Update standup log**: Update today's entry in `vault/06-ceremonies/standup/` with completion status and say-do score.
