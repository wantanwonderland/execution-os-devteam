Tech radar management. Display the current radar, add or move technologies between rings, and dispatch Wiz to research and recommend a ring.
...
- `/radar assess {tech}` — dispatch Wiz to research the technology and recommend a ring
...
- Source: `vault/04-decisions/log/` files where `type: tech-radar`
...
2. **Check for duplicate**: Search `vault/04-decisions/log/` for an existing radar entry for `{tech}`:
   ```bash
   grep -rl "{tech}" vault/04-decisions/log/
   ```
...
3. **Dispatch Wiz** with:
   ...
   - Instruction to write decision record to `vault/04-decisions/log/`
...
2. **Find existing entry**: Search `vault/04-decisions/log/` for the technology. If not found, suggest `/radar add` instead.
...
2. **Present research briefing**: Show Wiz's full assessment: recommendation, evidence table, confidence level, gaps.

3. **Prompt for action**:
   ```
   Wiz recommends: {Ring} ({confidence} confidence)
