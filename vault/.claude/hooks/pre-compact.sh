...
STATE_FILE="$SCRIPT_DIR/../coordinator-state.json"
...
SDD_TASK=""
SDD_PHASE=""
if [ -f "$SCRIPT_DIR/../sdd-state.json" ]; then
  SDD_TASK=$(python3 -c "import json; d=json.load(open('$SCRIPT_DIR/../sdd-state.json')); print(d.get('task',''))" 2>/dev/null || echo "")
  SDD_PHASE=$(python3 -c "import json; d=json.load(open('$SCRIPT_DIR/../sdd-state.json')); print(d.get('current_phase',''))" 2>/dev/null || echo "")
fi
