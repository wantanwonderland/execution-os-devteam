...
```
Tell Hange to research real-time collaboration options:
...
**Hange** delivers a research briefing:
...
```
Tell Tanjiro to add WebSocket-based presence indicators showing who's viewing the same document
```

Tanjiro evaluates, designs, and builds it directly.

### For a large feature:

```
Tell Tanjiro to plan the real-time collaboration feature using the fullstack workflow
```

Tanjiro uses `fullstack-workflow` skill:
...
2. **Design**:
   ...
   - **Ochaco** designs the collaboration UI (cursor colors, presence avatars, "typing..." indicators)
3. **Implement**: database → backend → frontend → tests (in that order)
4. **Review**: Levi reviews code, Killua runs tests, Itachi scans security
...
**Erwin** presents velocity baseline and asks for sprint goals. You add:
...
## Stage 6: Handle Requirement Changes Mid-Sprint
...
```
Tell Hange to research how to add inline commenting to a Yjs + ProseMirror setup. What's the effort? Does it change our architecture?
```

### Decide whether to include in current sprint:

```
/decide Adding inline comments to the current sprint. Hange confirmed Yjs supports annotation marks natively. Estimated 3 story points — fits within our remaining velocity.
```
...
Tell Tanjiro to fix this — use a hash of the user ID to assign deterministic colors instead of array index.
```
...
| **Research** | Tell Hange to research... | Research briefing with sources + gaps |
...
| **Design** | `/design` | Ochaco's aesthetic direction |
| **Build** | Tell Tanjiro to build... | Full-stack implementation |
| **Test** | `/test unit`, `/test browser` | Multi-layer testing |
| **Review** | Tell Levi to review PR #N | 4-angle code review |
...
3. **Let Hange research before you commit.** A 5-minute research briefing prevents a 5-day wrong turn.

4. **Sprint goals are commitments.** Erwin tracks say-do alignment. Under-promise and over-deliver.
