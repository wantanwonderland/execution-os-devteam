...
```
Tell Tanjiro to scaffold a new Laravel 11 project with:
- MySQL database
...
```
Tell Tanjiro to design a database for an e-commerce platform with:
...
Tanjiro generates Laravel migrations:
...
```
Tell Tanjiro to build RESTful API endpoints for products:
...
```
Tell Tanjiro to build the Vue.js admin dashboard with:
...
Tell Levi to review PR #5    # Code review
...
|------|---------|---------|
| Scaffold project | Tanjiro | `/new` |
| Database design | Tanjiro | natural language request |
| Generate migrations | Tanjiro + L | `/migrate create` |
| API development | Tanjiro | natural language request |
| API documentation | L | `/api gen` |
| Frontend design | Ochaco | `/design` |
| Frontend build | Tanjiro + Ochaco | natural language request |
| Code review | Levi | via `/pr-queue` |
