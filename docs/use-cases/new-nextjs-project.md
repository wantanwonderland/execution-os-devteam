...
Wantan dispatches **Conan** to scaffold a Next.js app. The wizard asks:
...
Conan creates the full project structure, installs dependencies, verifies the build passes, and starts the dev server.
...
Wantan dispatches **Rohan** who picks a bold aesthetic direction:
...
Rohan generates the full landing page component with responsive breakpoints.
...
```
Tell Conan to design the database for a project management app with users, projects, tasks, and comments
```

Wantan dispatches **Conan** using the `database-design` skill:
...
```
Tell Conan to set up JWT authentication with login, register, and role-based access
```

Conan uses the `auth-patterns` skill:
...
```
Tell Conan to build the task board feature — a Kanban-style board where users can create, move, and assign tasks within a project
```

Conan uses the `fullstack-workflow` skill:
...
```
Tell Diablo to review PR #3
```

**Diablo** does a 4-angle review:
...
- **Readability**: Suggests renaming `handleDrop` to `handleTaskColumnCwiz`

**Cleanliness: 7/10** — Cwizs requested.
...
**Killua** runs:
- **Unit tests**: 45 tests, 43 passed, 2 failed (the race condition Diablo flagged)
