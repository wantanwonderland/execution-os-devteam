...
```
Session 1: Diablo reviews PR #42, finds auth bug
  → Observation saved: "Diablo found missing auth check in /api/users/:id"

Session 2 (next day): You ask about the users endpoint
  → Wantan queries wantan-mem before dispatching
  → Injects context: "Diablo flagged an auth issue here yesterday"
...
```
What has Diablo flagged in the last 7 days?
```

Wantan uses the `agent_query` MCP tool:

```
agent_query(agent="diablo", days=7)
```
...
|-----|--------|-------|--------------------------------------------------|------------|
| 142 | review | diablo  | PR #89: Missing input validation on /api/orders  | 2026-03-28 |
| 138 | review | diablo  | PR #87: Clean refactor, approved 9/10            | 2026-03-27 |
| 135 | review | diablo  | PR #85: SQL injection risk in search endpoint    | 2026-03-26 |
```
...
Before dispatching Wiz + Diablo, Wantan **automatically searches wantan-mem**:
...
| 112 | review   | diablo   | Approved auth middleware PR — bcrypt + httpOnly       | 2026-02-20 |
```
...
```
    [86] 2026-02-14 | wiz (research): JWT vs Session comparison — JWT wins for multi-platform
    [87] 2026-02-14 | wiz (research): Refresh token strategies — rotation recommended
...
>>> [89] 2026-02-15 | wantan (decision): Chose JWT over sessions for SPA + mobile
    ...
    [91] 2026-02-17 | conan (insight): Auth middleware implemented with bcrypt + argon2
    [92] 2026-02-20 | diablo (review): Auth PR approved — clean implementation 9/10
```
...
**Kazuma** queries wantan-mem for patterns:
...
This becomes a retro action item → tech debt item → Senku designs the fix → Conan implements.
...
**Session 2 (Tuesday):**
```
Tell Conan to upgrade jsonwebtoken to v10.0.1 and update all imports
```

Before Conan starts, Wantan injects Itachi's finding:
...
Conan starts with full context — no re-investigation needed.

**Session 3 (Wednesday):**
```
Tell Diablo to review PR #95
```

Diablo sees the jsonwebtoken upgrade PR. Wantan injects:

> **Context from memory**: This PR addresses CVE-2026-5678 flagged by Itachi. Conan upgraded from v9.x to v10.0.1 yesterday. Breaking changes: `verify()` now returns a Promise instead of using callbacks.

Diablo focuses on verifying the migration is correct, not re-discovering why the change was made.
