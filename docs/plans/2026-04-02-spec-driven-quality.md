...
**Architecture:** One new skill (`spec-driven-dev`) defines the 5-phase mandatory workflow. One new rule (`sdd-enforcement`) makes it non-optional. Updates to 4 existing agents (Tanjiro, Killua, Levi, Byakuya) add enforcement constraints. No new agents — this is process, not headcount.
...
```
plugin/agents/tanjiro.md      # Add: refuses to code without approved spec
plugin/agents/killua.md       # Add: writes failing tests BEFORE implementation
plugin/agents/levi.md         # Add: independent review (never reviews own code)
...
**Who**: Wantan asks the user, Hange researches if needed, Senku reviews architecture
...
**Tanjiro MUST refuse to write code if this gate has not passed.**
...
**Killua owns this phase. Tanjiro does NOT write tests — the implementer never writes their own tests.**

---

## Phase 3: IMPLEMENT

**Who**: Tanjiro writes the minimum code to make tests pass
...
**Pre-commit hooks enforce this automatically.** If hooks fail, the commit is rejected and Tanjiro must fix before retrying.

---

## Phase 4: REVIEW

**Who**: Levi reviews (NEVER the same agent that wrote the code)
**Output**: Review with verdict: APPROVE / CHANGES REQUESTED
**Gate**: Levi must approve before proceeding

### Review Checklist

Levi checks against the spec (not general "is this good code"):
...
```
GATE 4: Has Levi approved?
  APPROVED → proceed to Phase 5
  CHANGES REQUESTED → Tanjiro fixes, then back to Gate 3 (re-test + re-review)
  Never skip this gate. Self-review is not a substitute.
```

**Critical: The agent that wrote the code (Tanjiro) NEVER reviews its own output. Levi is always a separate dispatch.**
...
User request
  ...
  → Phase 4: REVIEW (independent check) → Gate 4: Levi approves
...
| Agent writes tests AND code | Won't catch its own logical errors | Killua tests, Tanjiro implements |
| Agent reviews its own code | Blind to own assumptions | Levi reviews (always separate) |
...
## Task 3: Update Tanjiro — Refuse Coding Without Spec

**Files:**
- Modify: `plugin/agents/tanjiro.md`

- [ ] **Step 1: Read current Tanjiro and add SDD constraint**

Read `plugin/agents/tanjiro.md`. Add this section BEFORE the existing `## Constraints` section:

```markdown
## SDD Enforcement

Tanjiro follows Spec-Driven Development for ALL non-trivial work.

**Hard rule: Tanjiro REFUSES to write implementation code without:**
1. An approved spec (Phase 1 complete, user said "go")
2. Failing tests from Killua (Phase 2 complete)

If dispatched without these, Tanjiro responds:
> "I need a spec before I can implement this. Let's write one first — what are the acceptance criteria?"

Then helps create the spec (or defers to Hange for research).

**After implementation, Tanjiro does NOT self-review.** Levi reviews independently.

**Maximum 5 files per task.** If the spec requires more, Tanjiro splits into subtasks.
```
...
- NEVER review own code — always defer to Levi
...
```bash
git add plugin/agents/tanjiro.md
git commit -m "feat(tanjiro): enforce SDD — refuse coding without spec + tests"
```
...
**Killua writes tests. Tanjiro writes implementation. They are NEVER the same agent.**
...
- Never let the implementing agent (Tanjiro) write their own tests
...
## Task 5: Update Levi — Independent Review Enforcement

**Files:**
- Modify: `plugin/agents/levi.md`

- [ ] **Step 1: Read current Levi and add independent review constraint**

Read `plugin/agents/levi.md`. Add this section BEFORE `## Constraints`:

```markdown
## SDD Phase 4: Independent Review

In Spec-Driven Development, Levi owns Phase 4 — reviewing code he did NOT write.

**Critical rule: Levi NEVER reviews code that Levi wrote.** This prevents the #1 AI quality failure: self-review blindness (same agent misses its own logical errors).
...
- CHANGES REQUESTED → specific issues, Tanjiro fixes and re-submits
```
...
```bash
git add plugin/agents/levi.md
git commit -m "feat(levi): enforce independent review — never self-review, check against spec"
```
...
```bash
for agent in tanjiro killua levi byakuya; do
...
- [ ] Tanjiro updated — refuses code without spec + tests, max 5 files, shows evidence
- [ ] Killua updated — test-first enforcement, never same agent as implementer
- [ ] Levi updated — independent review, never self-review, check against spec
